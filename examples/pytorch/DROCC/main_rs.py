...
from data_process_scripts.process_rs import RS_Dataset
from edgeml_pytorch.trainer.drocc_trainer import DROCCTrainer

class RS_LeNet(nn.Module):

    def __init__(self):
        super(RS_LeNet, self).__init__()
...
def main():
    # Loading RS Dataset
    dataset = RS_Dataset("data", args.normal_class)
    train_loader, test_loader = dataset.loaders(batch_size=args.batch_size)

    # Using LSTM model check dimension
    # model = LSTM_FC(input_dim=6, num_classes=1, num_hidden_nodes=args.hd).to(device)
    
    # Using CNN model
    model = RS_LeNet().to(device)
