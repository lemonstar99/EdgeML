...
from data_process_scripts.process_hr import HR_Dataset
...
class HR_LeNet(nn.Module):

    def __init__(self):
        super(HR_LeNet, self).__init__()

        self.rep_dim = 64
        self.pool = nn.MaxPool1d(2, 2)

        self.conv1 = nn.Conv1d(200, 256, 3, bias=False, padding=2)
        self.bn2d1 = nn.BatchNorm1d(256, eps=1e-04, affine=False)
        self.conv2 = nn.Conv1d(256, 128, 4, bias=False, padding=2)
        self.bn2d2 = nn.BatchNorm1d(128, eps=1e-04, affine=False)
        self.conv3 = nn.Conv1d(128, 64, 3, bias=False, padding=2)
        ...
        self.fc3 = nn.Linear(16, 2, bias=False)

    def forward(self, x):
        # x = x.unsqueeze(0)
        print("1: ", x.size()) # [n, 11932, 3]
        x = self.conv1(x)
        print("2: ", x.size()) # [n, 5000, ]
...
def main():
    # Loading HR Dataset
    dataset = HR_Dataset("data", args.normal_class)
    train_loader, test_loader = dataset.loaders(batch_size=args.batch_size)

    # Using LSTM model check dimension
    # model = LSTM_FC(input_dim=3, num_classes=2, num_hidden_nodes=args.hd).to(device)
    
    # Using CNN model
    model = HR_LeNet().to(device)
