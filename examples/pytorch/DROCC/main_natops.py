...
from data_process_scripts.process_natops import NATOPS_Dataset
from edgeml_pytorch.trainer.drocc_trainer import DROCCTrainer

class NATOPS_LeNet(nn.Module):

    def __init__(self):
        super(NATOPS_LeNet, self).__init__()
...
def main():
    # Loading NATOPS Dataset
    dataset = NATOPS_Dataset("data", args.normal_class)
    train_loader, test_loader = dataset.loaders(batch_size=args.batch_size)

    # Using LSTM model check dimension
    # model = LSTM_FC(input_dim=24, num_classes=1, num_hidden_nodes=args.hd).to(device)
    
    # Using CNN model
    model = NATOPS_LeNet().to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--normal_class', type=int, default=0, metavar='N',
                    help='NATOPS normal class index')
