...
from data_process_scripts.process_ep import EP_Dataset
from edgeml_pytorch.trainer.drocc_trainer import DROCCTrainer

class EP_LeNet(nn.Module):

    def __init__(self):
        super(EP_LeNet, self).__init__()
    ...
    def forward(self, x):
        # print("0: ", x.size()) # [34, 206, 3]
        x = x.unsqueeze(3)
        # print("1: ", x.size()) # [34, 206, 3, 1]
        x = self.conv1(x)
        # print("2: ", x.size()) # [34, 32, 5, 3]
        x = self.pool(F.leaky_relu(self.bn2d1(x)))
        # print("3: ", x.size()) # [34, 32, 2, 1]
        x = self.conv2(x)
        # print("4: ", x.size()) # [34, 64, 4, 3]
        x = self.pool(F.leaky_relu(self.bn2d2(x)))
        # print("5: ", x.size()) # [34, 64, 2, 1]
        x = self.conv3(x)
        # print("6: ", x.size()) # [34, 128, 4, 3]
        x = self.pool(F.leaky_relu(self.bn2d3(x)))
        # print("7: ", x.size()) # [34, 128, 2, 1]
        x = x.view(x.size(0), -1)
        # print("8: ", x.size()) # [34, 256]
        # x = self.fc1(x)
        x = F.leaky_relu(self.fc1(x))
        # print("9: ", x.size()) # [34, 64]
...
def main():
    ...
    """
    # Loading EP Dataset
    dataset = EP_Dataset("data", args.normal_class)
    train_loader, test_loader = dataset.loaders(batch_size=args.batch_size)
